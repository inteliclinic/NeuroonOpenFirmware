...
#define TWI_REGISTER(name, code)\
  ...
  nrf_drv_twi_enable(&twi_instance_##name)

#define TWI_SEND_DATA(name, address, in_buffer, len)\
  nrf_drv_twi_tx(&twi_instance_##name, (uint8_t)address, (uint8_t *)in_buffer, (uint8_t)len, false)

/*
 *#define TWI_READ_DATA(name, in_buffer, out_buffer, len)\
 *  nrf_drv_twi_tx(&twi_instance_##name, (uint8_t)address, (uint8_t *)in_buffer, (uint8_t)len, true);
 */
